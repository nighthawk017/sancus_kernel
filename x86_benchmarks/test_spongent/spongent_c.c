...
//TODO LIST
// check if nullptr and NULL are the same
//
...
void c_copy_n(void *first, int count, void* result) {
 	memcpy(result,first,count/8); //TODO check if correct
}
...
void c_fill_n(BitSequence *start, int size, int value) {
	memset(start, value, size);	
}
/* check if this is correct and makes any sense */
int c_equal(BitSequence* start, BitSequence* stop, BitSequence* expectedValue) {
