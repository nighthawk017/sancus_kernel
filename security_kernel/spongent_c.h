...
#define rate        (SW_RATE + 2) //18
#define capacity    (WIDTH - rate) //176 - 18 = 158
#define hashsize    SW_SECURITY

#define BITS_TO_BYTES(x) (x / 8 + (x % 8 != 0))

#define R_SizeInBytes 	BITS_TO_BYTES(rate)
#define nBits 			(capacity + rate) //176
#define nSBox 			nBits/8 // 22
...
HashReturn SM_FUNC(kernel)  Init(hashState *state, BitSequence *hashval);
// TODO check why there were parameters that weren't used in the signature and how that works
HashReturn SM_FUNC(kernel)  Absorb(hashState *state);//, const BitSequence *data, DataLength databitlen);
...
void SM_FUNC(kernel) initialize_box(void);
