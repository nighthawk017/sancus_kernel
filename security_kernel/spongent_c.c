...
long long SM_FUNC(kernel) c_min(long long a, long long b) {
	if(a > b)
		return b;
	else
		return a;
...
int SM_FUNC(kernel) defaultPi(int i)
{
...
HashReturn SM_ENTRY(kernel) SpongentWrap(const BitSequence* key,
...
HashReturn SM_ENTRY(kernel) SpongentUnwrap(const BitSequence* key,
                          const BitSequence* ad, DataLength adBitLength,
                          const BitSequence* input, DataLength bitLength,
                          BitSequence* output,
                          const BitSequence* expectedTag)
{
    ...
    if (!c_equal(&tag[0], &tag[TAG_SIZE_BYTES-1], expectedTag)) //std::equal(std::begin(tag), std::end(tag), expectedTag))
        return BAD_TAG;

    return SUCCESS;
}

HashReturn SM_ENTRY(kernel) SpongentMac(const BitSequence* key,
                       const BitSequence* input, DataLength bitLength,
                       BitSequence* mac)
{
...
//--------------------------------------------------------------------------------------------
  BitSequence msg[256] = "Sponge + Present = Spongent";
	BitSequence key[64] = "12345678";      
...
//	BitSequence msg[256] = {'S', 'p', 'o', 'n', 'g', 'e', ' ', '+', ' ', 
//								'P', 'r', 'e', 's', 'e', 'n', 't', ' ', '=', ' ', 
//								'S', 'p', 'o', 'n', 'g', 'e', 'n', 't'};   	

//	BitSequence msg[256] = {0x53, 0x70, 0x6F, 0x6E, 0x67, 0x65, 0x20, 0x2B, 
...
//								0x20, 0x3D, 0x20, 0x53, 0x70, 0x6F, 0x6E, 0x67, 0x65, 0x6E, 0x74};
	...
	SpongentHash(msg,databitlen,hashval);
	//print hash
	debug_puts("\nHash\t\t:");
...
void SM_ENTRY(kernel) generateTestVectors2(void)
{
	int i;
	BitSequence hashval[hashsize/8]={0};
...
//								0x20, 0x3D, 0x20, 0x53, 0x70, 0x6F, 0x6E, 0x67, 0x65, 0x6E, 0x74};
	...
	SpongentWrap(key, msg, databitlen, NULL, 0, NULL, hashval,0);
	//print hash
	debug_puts("\nHash\t\t:");
	for(i=0; i<hashsize/8; i++)
		debug_print_int("%X",hashval[i]);
	debug_puts("\n");		
}
